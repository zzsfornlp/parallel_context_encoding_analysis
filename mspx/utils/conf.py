...
class Conf(Serializable):
    ...
    # compare with the default one! todo(+N): currently cannot support complex ones!
    def diff_with_default(self):
        # --
        def _is_ok_type(_v):
            return _v is None or isinstance(_v, (int, float, str, bool, list, dict))
        # --
        _df = self.__class__()
        ret = {}
        for k, v in self.__dict__.items():
            v2 = getattr(_df, k)
            if v != v2 and _is_ok_type(v) and _is_ok_type(v2):
                ret[k] = v
        return ret
    ...
    def get_subconf(self, s: str, ff=None, T=None, only_one=False, name_prefix="", **kwargs):
        specs = Conf.typed_convert(s, list)
        if T is None:
            T = Registrable
        ...
        subconfs = []
        for spec in specs:
            _parts = spec.strip().split(Conf.SEP_NV)
            if len(_parts) > 0:  # ignore empty ones!
                if len(_parts) == 1:  # sig as key
                    _parts = [_parts[0]] + _parts
                _name = name_prefix + _parts[0]
                cc = ff(*_parts[1:])
                ...
                subconfs.append((_name, cc))
        # --
        zlog(f"Get subconfs for {self}: {subconfs}")
        if only_one:
            assert len(subconfs) == 1, f"Get multiple subconfs: {subconfs}"
            return subconfs[0][1]
        else:
            return [z[0] for z in subconfs]  # only return the names
        # --
...
class GlobalConf(Conf):
    ...
    def add_subconf(self, name: str, value: Conf):
        ...
        if hasattr(self, name):
            zwarn(f"Overwrite subconf {name}")
