...
__all__ = [
    "Logger", "zlog", "zwarn", "zwarn_once", "zfatal", "zcheck",
]
...
class Logger:
    ...
    @staticmethod
    def init(files: Iterable = (), level=0, use_magic_file=False, log_cached=False, log_append=False, log_main_dir=None):
        from .system import auto_prep_path
        ...
        if log_main_dir:
            log_files.extend([auto_prep_path(path_file=[log_main_dir, f]) for f in files if f])
        else:
