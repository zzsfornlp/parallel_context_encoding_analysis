...
class MspUtilsConf(Conf):
    def __init__(self):
        ...
        self.main_dir = ""  # main dir for various things
        self.str_header = ""  # print this after init-everything
        # random
...
def init(utils_conf: MspUtilsConf, extra_files: List=None):
    ...
    Logger.init(flist, level=utils_conf.log_level, use_magic_file=utils_conf.log_magic_file,
                log_cached=utils_conf.log_cached, log_append=utils_conf.log_append, log_main_dir=GET_MAIN_DIR())
    ...
    zlog(f"ZHEADER: {utils_conf.str_header}")
    check_env_info()
...
def ZDEBUG():
    global _FLAG_ZDEBUG
    if _FLAG_ZDEBUG is None:
        flag = get_global_conf(['utils', 'zdebug'])
        flag = flag or bool(os.environ.get("ZDEBUG", False))
        _FLAG_ZDEBUG = flag
    return _FLAG_ZDEBUG
...
def GET_ENV_VAR(*keys: str, df=None):
    for k in keys:
        if k in _GLOBAL_FLAGS:
            return _GLOBAL_FLAGS[k]  # directly return it
        if k in os.environ:
            _GLOBAL_FLAGS[k] = os.environ[k]  # cache
            return _GLOBAL_FLAGS[k]
    return df

def GET_MAIN_DIR():
    main_dir = get_global_conf(['utils', 'main_dir'])
    if main_dir is None:
        main_dir = ""
    return auto_prep_path(path_dir=main_dir)  # make it if not existing
...
def init_everything(main_conf: Conf, args: Iterable[str]=None, add_utils=True, add_nn=True, sbase_getter=None, rank=None):
    ...
    if add_nn:
        ...
        _is_main_process = BK.is_main_process()
    # --
    # then actual init
    ...
    if add_utils:
        # write conf?
        if utils_conf.conf_output and (rank is None or _is_main_process):
