...
__all__ = [
    "Timer", "AccEvalEntry", "F1EvalEntry", "StatRecorder", "get_singleton_global_recorder", "MyCounter", "stat_analyze",
]
...
class StatRecorder:
    def __init__(self, report_key: str = None, report_interval=0, report_start_timer=True):
        ...
        self.reset()
        # --
        self.last_report_val = 0.  # note: simply mark it as 0!
        if report_key and report_start_timer:  # start this one!
            x = self.timers[report_key]
            x.resume()  # start it right away!
        # --
    ...
    def summary(self, report=''):
        ...
        if _report:
            ...
            _time = self.timers[self.report_key].get_accu_time() + 1e-5
            zlog(f"{report}: {r} [{_time:.2f}s] [{(_count/_time):.2f}d/s]", timed=True)
            self.timers[self.report_key].resume()  # resume
        return r
...
# --
# light-weighted stat analyzer (convert data to pd table)
# -- data=List[(K=Dict,V=?)], note: must make sure all the keys are available
# -- f_label=lambda k: (label_x, label_y) or None, f_entry=lambda gdata: item
def stat_analyze(data, f_label, f_entry):
    from .utils import ZHelper
    f_label, f_entry = [(ZHelper.eval_ff(z) if isinstance(z, str) else z) for z in [f_label, f_entry]]
    table0 = {}  # (label_x, label_y) -> [gdata]
    for k, v in data:  # iter all data
        _key0 = f_label(k)  # (x,y)
        if _key0 is not None:
            if _key0 not in table0:
                table0[_key0] = []
            table0[_key0].append((k, v))
    # create table
    all_lx = sorted(set([z[0] for z in table0.keys()]))
    all_ly = sorted(set([z[1] for z in table0.keys()]))
    arr0 = [[None for _j in all_ly] for _i in all_lx]  # [X, Y]
    for ii, xx in enumerate(all_lx):
        for jj, yy in enumerate(all_ly):
            arr0[ii][jj] = f_entry(table0.get((xx, yy), []))
    df = pd.DataFrame(arr0, index=all_lx, columns=all_ly)
    return df
